...
type ReservationData interface {
	...
	UpdateReservation(reservationID string, newData map[string]interface{}) (*Reservation, error)
	DeleteReservation(id string) error
	GetReservationByID(id string) (*Reservation, error)
	IsRoomValid(roomID string) (bool, map[string]any, error)
}
...
type ReservationService interface {
	...
	UpdateReservation(id string, newData Reservation, token *jwt.Token) (*Reservation, error)
	DeleteReservation(id string, token *jwt.Token) error
	GetReservationByID(id string, token *jwt.Token) (*Reservation, error)
}
